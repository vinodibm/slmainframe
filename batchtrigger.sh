...
echo "/* " >> /root/tmpfile
curl -k -i -X PUT https://user:password@192.168.0.227:32208/zosmf/restjobs/jobs -H "Content-Type: text/plain" -H "X-IBM-Intrdr-Class: A" --data-binary @/root/tmpfile >> /root/startup
date >> /root/startup
